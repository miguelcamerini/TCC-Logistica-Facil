...
class Viagem(models.Model):
    ...
    # Datas que determinam quando os valores serão lançados no Caixa
    ...
    # Valores de Adiantamento e Quitação
    valor_adiantamento = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Valor do Adiantamento")
    valor_quitacao = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Valor da Quitação")
    
    media = models.DecimalField(max_digits=6, decimal_places=2, default=0.0, verbose_name="Média de Consumo (km/L)")
    descricao = models.TextField(blank=True, null=True, verbose_name="Descrição")

    #Propriedade calculada dinamicamente (não salva no banco)
    @property
    def valor_total(self):
        adiantamento = self.valor_adiantamento or Decimal('0.00')
        quitacao = self.valor_quitacao or Decimal('0.00')
        return adiantamento + quitacao
    ...
    def clean(self):
        ...
        if self.valor_adiantamento < 0:
            raise ValidationError({'valor_adiantamento': 'Valor do adiantamento não pode ser negativo'})
        if self.valor_quitacao < 0:
            raise ValidationError({'valor_quitacao': 'Valor da quitação não pode ser negativo'})
        if self.km_rodados < 0:
            raise ValidationError({'km_rodados': 'KM rodados não pode ser negativo'})
        
        
        # so adiciona a validação de datas de adiantamento e quitação se elas estiverem preenchidas
        # 2. VALIDAÇÃO ADIANTAMENTO: Data obriga Valor > 0 / Valor > 0 obriga Data
        v_adiantamento = self.valor_adiantamento or Decimal('0.00')
        
        if self.data_adiantamento and v_adiantamento <= 0:
            raise ValidationError({
                'valor_adiantamento': 'Informe o valor do adiantamento ao selecionar uma data.'
            })
        if v_adiantamento > 0 and not self.data_adiantamento:
            raise ValidationError({
                'data_adiantamento': 'Informe a data do adiantamento ao inserir um valor.'
            })

        # 3. VALIDAÇÃO QUITAÇÃO: Data obriga Valor > 0 / Valor > 0 obriga Data
        v_quitacao = self.valor_quitacao or Decimal('0.00')
        
        if self.data_quitacao and v_quitacao <= 0:
            raise ValidationError({
                'valor_quitacao': 'Informe o valor da quitação ao selecionar uma data.'
            })
        if v_quitacao > 0 and not self.data_quitacao:
            raise ValidationError({
                'data_quitacao': 'Informe a data da quitação ao inserir um valor.'
            })
        #---------
...
# ==========================================
# MODEL: CAIXA (Atualizado)
# ==========================================
class Caixa(models.Model):
    TIPO_CHOICES = [
        ('despesa', 'Despesa / Gasto (-)'),
        ('adiantamento', 'Recebimento - Adiantamento (+)'),
        ('quitacao', 'Recebimento - Quitação (+)'),
        ('receita_extra', 'Entrada Externa / Receita (+)'),
    ]

    usuario = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="Usuário", db_index=True)
    data = models.DateField(verbose_name="Data", db_index=True)
    
    valor_entrada = models.DecimalField(max_digits=10, decimal_places=2, default=0.00, verbose_name="Valor Entrada (+)")
    valor_saida = models.DecimalField(max_digits=10, decimal_places=2, default=0.00, verbose_name="Valor Saída (-)")
    
    tipo = models.CharField(max_length=20, choices=TIPO_CHOICES, verbose_name="Tipo de Lançamento")
    centro_de_custo = models.CharField(max_length=100, verbose_name="Centro de Custo / Veículo")
    
    viagem = models.ForeignKey('Viagem', on_delete=models.CASCADE, blank=True, null=True, verbose_name="Viagem Origem")
    descricao = models.TextField(verbose_name="Descrição")

    @property
    def mic_exibicao(self):
        return self.viagem.mic if (self.viagem and self.viagem.mic) else "-"

    @property
    def nota_fiscal_exibicao(self):
        return self.viagem.nota_fiscal if (self.viagem and self.viagem.nota_fiscal) else "-"

    def clean(self):
        # Trata None convertendo para 0.00 para evitar o TypeError de comparação com NoneType
        v_saida = self.valor_saida or 0.00
        v_entrada = self.valor_entrada or 0.00

        if self.tipo == 'despesa' and v_saida <= 0:
            raise ValidationError({'valor_saida': 'O valor da despesa deve ser maior que 0.'})
        elif self.tipo and self.tipo != 'despesa' and v_entrada <= 0:
            raise ValidationError({'valor_entrada': 'O valor da entrada deve ser maior que 0.'})

    def __str__(self):
        valor = self.valor_saida if self.tipo == 'despesa' else self.valor_entrada
        return f"{self.get_tipo_display()} - R$ {valor}"
