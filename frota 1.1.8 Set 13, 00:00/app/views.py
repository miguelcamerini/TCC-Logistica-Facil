...
from django.db.models import Avg, Q, ProtectedError, Sum
from django.http import HttpResponse
from django.shortcuts import get_object_or_404, redirect, render

from .models import Abastecida, Caixa, Funcionario, Veiculo, Viagem
    ...
    if viagem.data_adiantamento and viagem.valor_adiantamento and viagem.valor_adiantamento > 0:
        Caixa.objects.update_or_create(
            ...
            defaults={
                ...
                'nota_fiscal': viagem.nota_fiscal if hasattr(viagem, 'nota_fiscal') else '',
...
def lista_viagem(request):
    ...
    viagens_ativas = viagens.filter(status_da_viagem__in=['planejada', 'em_andamento']).order_by('-data_inicio')
    viagens_historico = viagens.filter(status_da_viagem__in=['concluida', 'quitada', 'cancelada']).order_by('-data_inicio')
...
def excluir_viagem(request, pk):
    ...
    if request.method == 'POST':
        ...
        if qtd_caixa > 0:
            messages.warning(request, f"A Viagem #{id_viagem} foi excluída e seus {qtd_caixa} lançamento(s) de frete vinculados no Caixa também foram removidos!")
        else:
...
@login_required
def home(request):
    viagens_qs = Viagem.objects.filter(usuario=request.user, status_da_viagem='em_andamento').order_by('-data_inicio')
    ...
    return render(request, 'home.html', {
        ...
        'tem_mais_abastecimentos': tem_mais_abastecimentos,
