...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btnToggle && valorOculto && valorReal) {
        let saldoVisivel = localStorage.getItem('saldo_visivel_home') === 'true';

        function atualizarVisibilidade(mostrar) {
            if (mostrar) {
        ...
        // Aplica o estado inicial carregado
        atualizarVisibilidade(saldoVisivel);

        // Evento de clique
        btnToggle.addEventListener('click', function() {
            saldoVisivel = !saldoVisivel;
            localStorage.setItem('saldo_visivel_home', saldoVisivel);
            atualizarVisibilidade(saldoVisivel);
        });
    }
});