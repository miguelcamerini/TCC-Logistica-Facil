...
def home(request):
    ...
    # Otimização de performance usando Sum do Banco
    totais = caixa_qs.aggregate(
        total_entradas=Sum('valor_entrada'),
        total_saidas=Sum('valor_saida')
    )
    total_entradas = totais['total_entradas'] or 0
    total_saidas = totais['total_saidas'] or 0
    saldo_caixa = total_entradas - total_saidas

    # 3. NOVO: Abastecimentos recentes (3 últimos)
    abastecidas_qs = Abastecida.objects.filter(usuario=request.user).order_by('-data_abastecida', '-id')
    ultimos_abastecimentos = abastecidas_qs[:3]
    tem_mais_abastecimentos = abastecidas_qs.count() > 3

    # 4. Funcionários contratados/ativos (3 últimos)
    ...
    # 5. Veículos operacionais (3 últimos)
    ...
    return render(request, 'home.html', {
        ...
        'ultimos_abastecimentos': ultimos_abastecimentos, # Novo
        'tem_mais_abastecimentos': tem_mais_abastecimentos, # Novo
...
class AbastecidaViagemForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'valor_abastecida': forms.NumberInput(attrs={'step': '0.01', 'placeholder': '0.00'}),
            'litros': forms.NumberInput(attrs={'step': '0.01', 'placeholder': '0.00'}),
            'nota_fiscal': forms.TextInput(attrs={'placeholder': 'Nº da Nota Fiscal (opcional)'}),
            'descricao': forms.Textarea(attrs={'rows': 3, 'placeholder': 'Ex: Posto Shell BR 116...'}),
        }
...
# ==========================================
# 2. FORMULÁRIO DE ABASTECIMENTO COMUM
# ==========================================
class AbastecidaComumForm(forms.ModelForm):
    class Meta:
        model = Abastecida
        ...
        widgets = {
            ...
            'litros': forms.NumberInput(attrs={'step': '0.01', 'placeholder': '0.00'}),
            'nota_fiscal': forms.TextInput(attrs={'placeholder': 'Nº da Nota Fiscal (opcional)'}),
            'descricao': forms.Textarea(attrs={'rows': 3, 'placeholder': 'Ex: Abastecimento de Pátio...'}),
        }
...
# HELPER DE SINCRONISMO
# ==========================================
def sincronizar_abastecimento(abastecimento, viagem_antiga=None):
    """
    Sincroniza os dados entre Abastecimento, Viagem e Caixa.
    """
    def recalcular_media_viagem(viagem_obj):
        if not viagem_obj:
            return
    ...
    # Recalcula a viagem antiga se houve alteração na edição
    if viagem_antiga and viagem_antiga != abastecimento.viagem:
        recalcular_media_viagem(viagem_antiga)

    # 2. LANÇA/ATUALIZA NO CAIXA
    veiculo_nome = str(abastecimento.veiculo) if abastecimento.veiculo else "FROTA"
    tag_busca = f"Abastecimento #{abastecimento.id}"
    ...
    caixa_item.valor_entrada = 0.00
    caixa_item.centro_de_custo = veiculo_nome
    caixa_item.nota_fiscal = abastecimento.nota_fiscal or ""
    caixa_item.descricao = f"{tag_busca} - {abastecimento.litros}L ({veiculo_nome})"
    caixa_item.save()
...
def add_abastecida_viagem(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sincronizar_abastecimento(abastecimento)

            messages.success(request, f"Abastecimento de {abastecimento.litros}L salvo com sucesso!")
            return redirect('lista_abastecida')
    else:
        form = AbastecidaViagemForm(user=request.user)
...
def add_abastecida(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Abastecimento comum de {abastecimento.litros}L salvo com sucesso!")
            return redirect('lista_abastecida')
    else:
        form = AbastecidaComumForm(user=request.user)
...
def edit_abastecida(request, pk):
    abastecimento = get_object_or_404(Abastecida, id=pk, usuario=request.user)
    viagem_antiga = abastecimento.viagem
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            abastecimento_salvo.save()
            sincronizar_abastecimento(abastecimento_salvo, viagem_antiga=viagem_antiga)
